---
results:true
---

#COMANDOS BÁSICOS

←[cor=150,130,0,1]function[/cor] [cor=150,130,0,1]setup(){}[/cor]: Executa o que estiver entre {} apenas uma vez;→
[cor=150,130,0,1]function[/cor] [cor=150,130,0,1]draw(){}[/cor]: Executa o que estiver entre {} sem parar;
[cor=150,130,0,1]background[/cor]([cor=240,0,0,1]R[/cor],[cor=0,240,0,1]G[/cor],[cor=0,0,240,1]B[/cor]) 

← &emsp;&emsp; O objetivo desse site é facilitar o acesso aos conteúdos *(principalmente de Física)*, de forma gamificada e divertida, onde você aluno possa aprender de uma forma diferente.→

**→[cor=10,10,200,1]Abaixo uma simulação do modelo atômico clássico:[/cor]←**
←[simulacao=show_atom]←

**→[cor=200,10,10,1]Abaixo uma interação do campo elétrico de dois íons de carga oposta:[/cor]←**
→[simulacao=pn_field]→

